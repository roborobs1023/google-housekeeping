...
function DeleteFolderByDate() {
      ...
      while (Folders.hasNext()) {
        ...
        //  Logger.log ('\n' + "Search Folder: " + FolderName)
          if (!hasFile && !hasFolder) {            
            arrFolderIDs.push(FolderID);
            Logger.log ('\n' + FolderName + " Has No Child Folders or Files and was removed(GetFolders)" + '\n')
            Folder.setTrashed(true)
          } 
          if (hasFolder){
            arrHasChildFolderIDs.push(FolderID);
          }
        }
      }
    }
    return arrHasChildFolderIDs;
  }      
  
  function deleteEmptyFolders() {
    var arrayIDs = GetFolders()
    var x = arrayIDs.length;
    for (i = 0; i < arrayIDs.length; i++){
      var parentFolder = DriveApp.getFolderById(arrayIDs[i]);
      var folders = parentFolder.getFolders();
      var FolderName = parentFolder.getName();
  
    //  Logger.log ('\n' + FolderName + " Has No Child Folders or files and was removed. (Recursive)")
    ...
    if (x > 0){
      //Send email to Administrator - Replace email with that of the desired contact. Email will come from the google account associated with the script.
      var recipient = "rtucker@cmslaser.com";
      var subject = 'Empty Folders Found on Google Drive removed.';
      var body = Logger.getLog();
      MailApp.sendEmail(recipient, subject, body);   
    }
  };
  ...
  function recurseFolder(parentFolder, folder) {
    ...
    var hasFile = folder.getFiles().hasNext();               // get the list of files in the root of that folder
    var hasFolder = folder.getFolders().hasNext();               // get the list of sub folders in the root of that folder
    var FolderName = folder.getName();
  
    if (!hasFile && !hasFolder) {            
      folder.setTrashed(true)
      Logger.log ('\n' + FolderName + " Has No Child Folders and has been removed.(Recursive)")
