...
function GetFolderIDs(folder, file) {
  ...
  for (var i = 0; i < arrSearchFolderIDs.length; i++) {
       ...
       while (Folders.hasNext()) {
         ...
         var hasFiles = Folder.getFiles().hasNext();        // get list of files in the root of folder
   ...
   function ConvertFiles (file, folder){

    var arrSearchFolders = [];
    var arrSearchFolderIDs = [];

    arrSearchFolderIDs = GetFolderIDs();
     for (var i = 0; i < arrSearchFolderIDs.length; i++) {
         SearchFolder = DriveApp.getFolderById(arrSearchFolderIDs[i]);
       ...
       while (Folders.hasNext()) {
         ...
         //Replace Templates with folders you would like the script to ignore and not remove.
         //Put the ID of each Folder into the arrFolderIDs array.
         if (FolderName != "Templates"){
